...
void P_ProjectSource(edict_t *ent, vec3_t distance, vec3_t forward,
...
void Weapon_Generic(edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST,
    ...
    Weapon_Generic2(ent, FRAME_ACTIVATE_LAST, FRAME_FIRE_LAST, FRAME_IDLE_LAST,
            FRAME_DEACTIVATE_LAST, pause_frames, fire_frames, fire);
    ...
    if ((CTFApplyHaste(ent)
            || ((Q_stricmp(ent->client->pers.weapon->pickup_name, "Grapple")
                    == 0) && (ent->client->weaponstate != WEAPON_FIRING)))
